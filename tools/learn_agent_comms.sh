...
LEARN_ROOT="$REPO_ROOT/Docs/learn/agent_comms"
SUBSET_DIR="$LEARN_ROOT/subsets/agent_comms"
MANIFEST="$SUBSET_DIR/source_manifest.tsv"
SOURCE_ROOT="${AGENT_COMMS_SOURCE_ROOT:-$HOME/GitHub}"
...
映射目标（除通用 Rust 映射外必须覆盖）：zenpi 的 DAG 编排需求——worker 负责 DAG 节点时需要与 parent/grandparent/直接 sibling/直接 child 通信；节点自身及其全部 child/grandchild 全绿才允许 close，否则保活并派生新 worker 处理新工作。请在笔记的 zenpi Rust 映射中明确：可复用的通信原语（消息/邮箱/事件/协议）、会话父子关系、保活与派生的最小机制，以及与本项目既有 src/headless.rs、src/core.rs、src/session.rs、src/runtime.rs 的落点。
...
main() {
  ...
  if [ ! -d "$SOURCE_ROOT" ]; then echo "missing source root: $SOURCE_ROOT" >&2; exit 1; fi
  first_source="$(manifest_query all | head -1 | cut -f2)"
  if [ -z "$first_source" ] || [ ! -f "$SOURCE_ROOT/$first_source" ]; then
    echo "missing first source: $SOURCE_ROOT/$first_source" >&2
    exit 1
  fi
  rm -f "$LOG_DIR/.inflight"
