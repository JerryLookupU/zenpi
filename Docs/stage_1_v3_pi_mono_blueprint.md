...
- [ ] **ZS1-174** — 二层 worktree 并发数字居中与双击内联编辑：`↑ N ↓` 的当前值必须在按钮块内水平居中；双击数字直接进入内联数字编辑（只接受有界整数），Enter 提交、Esc 取消，提交后写回该 worktree 的最大并发数并与键盘调整共享同一校验；layer `L3` | Depends: ZS1-146,ZS1-152,ZS1-167,ZS1-169 | Owner scope: 二层并发控件渲染与内联编辑 owner | Owned paths: `src/tui.rs`, `tests/tui_interaction.rs`, `tests/tui_bentobox.rs` | Validators: G-CODE；cargo test --locked --test tui_interaction --test tui_bentobox | Rollback: 撤回数字居中与双击编辑，恢复原并发控件 | Estimate: 居中 + 双击内联数字编辑与提交/取消 | Estimated LOC: 700
- [ ] **ZS1-175** — Resources 多 headless 网格观察模式：双击 Resources 面板进入全屏观察模式（再次双击返回）；观察模式以正方形格子阵列展示每个 headless worker 的实时状态（phase/CPU/RSS/i/o 摘要），列数必须取 2 的整数次幂，行/列组合尽量贴近当前显示区域长宽比，格子保持正方形（不足一格的余量均匀留白）；layer `L3` | Depends: ZS1-159,ZS1-162 | Owner scope: Resources 观察模式与网格布局 owner | Owned paths: `src/tui.rs`, `src/resources.rs`, `tests/tui_interaction.rs`, `tests/resources.rs` | Validators: G-CODE；cargo test --locked --test tui_interaction --test resources | Rollback: 撤回网格观察模式，恢复普通 Resources 面板 | Estimate: 2 的幂列数+宽高比适配+正方形格子+双击切换 | Estimated LOC: 1600
- [x] **ZS1-176** — 集成 jerrylookup PR #5–#10（以 PR 为主解决冲突）：合并 chat stream 空 tool name/id 容错、/new 未决操作、OpenAI wire 输出上限、未知结果解除输入封锁与 transcript 修复、/recovery abandon-all、recovery UX SPEC；冲突一律以 PR 版本为准，集成后跑对应测试并 push；layer `L5` | Depends: ZS1-119 | Owner scope: 外部 PR 集成与冲突解决 owner | Owned paths: `src/backend.rs`, `src/backend/chat_stream.rs`, `src/core.rs`, `src/headless.rs`, `src/input_queue.rs`, `src/slash.rs`, `src/tui.rs`, `src/view_model.rs`, `tools/zenpi_errorlog.py`, `Docs/feat-recovery-ux-and-logging.md` | Validators: G-CODE；cargo test --locked --test backend --test core_session --test session_new --test slash --test stage1_model_registry | Rollback: 撤回 PR 集成提交，保留主线历史 | Estimate: 合并 PR #5–#10，冲突以 PR 为准，测试通过后 push | Estimated LOC: 0
- [x] **ZS1-177** — 完整热区模型：显式 `无热区/Conversation/Resources/Arch/Gantt/Shell` 六态；焦点决定热区，热区决定键盘与鼠标行为矩阵（Conversation 可输入 prompt/slash；Arch 独立输入 prompt/slash；Resources 上下选择+Enter 进入/展开+双击观察模式；Gantt 支持键盘与滚轮/触控板上下拨动；Shell 原始 PTY 输入；无热区只保留全局键），并在 footer 显示当前热区与其快捷键提示；Esc 从 Arch 回 Conversation、从 Resources/Gantt 退到无热区；layer `L3` | Depends: ZS1-172,ZS1-174,ZS1-175 | Owner scope: 热区状态机与分区键鼠行为 owner | Owned paths: `src/tui.rs`, `src/layout.rs`, `tests/tui_interaction.rs`, `tests/tui_bentobox.rs` | Validators: G-CODE；cargo test --locked --test tui_interaction --test tui_bentobox | Rollback: 撤回六态热区，恢复三态 HotZone | Estimate: 六态映射+每区快捷键矩阵+无热区+footer 提示 | Estimated LOC: 1800
- [x] **ZS1-178** — Session 自定义命名与 `-s` 恢复：当前 session 在 header（logo 右侧、workspaces 标签左侧）显示名字并支持双击内联重命名；提交时校验有界名字并检查与缓存 zenpi session 名字重复，重复则友好提示并要求重新输入；名字以 append-only `session_renamed` 事件持久化并投影进 SessionSummary/SessionCatalogEntry；`zenpi -s [name 或 id]` 优先按名字、再按 session id 解析到 journal 路径并恢复（无需输入 hash）；layer `L3` | Depends: ZS1-176 | Owner scope: session 命名/解析与 header 重命名 owner | Owned paths: `src/session.rs`, `src/core.rs`, `src/tui.rs`, `tests/session_new.rs`, `tests/tui_interaction.rs`, `tests/config.rs` | Validators: G-CODE；cargo test --locked --test session_new --test tui_interaction --test config | Rollback: 撤回命名事件与 -s 解析，保留路径式 --session | Estimate: 命名事件投影+双击重命名+重名检查+-s 名字/id 解析 | Estimated LOC: 1900
- [x] **ZS1-179** — 双击 Ctrl-C 强杀与可恢复性：Ctrl-C 按热区区分语义——Conversation/Arch 单次为优雅中断请求、双击（同一窗口内）为强制终止；Resources/Gantt/无热区不产生文本输入，单次提示“再按一次强制终止”，双击强制终止当前 job；Shell 单次 Ctrl-C 仍转发 PTY，双击在存在活动 job 时额外强制终止；强制终止清理队列与审批、保留 journal，使 `zenpi -s [name 或 id]` 可续接；空闲时双击 Ctrl-C 作为退出确认；layer `L3` | Depends: ZS1-177,ZS1-178 | Owner scope: 中断语义与恢复安全 owner | Owned paths: `src/tui.rs`, `src/runtime.rs`, `tests/tui_interaction.rs`, `tests/runtime.rs` | Validators: G-CODE；cargo test --locked --test tui_interaction --test runtime | Rollback: 撤回双击强杀，恢复单次 Interrupt/Quit | Estimate: 双击窗口+分区中断语义+强杀清理+可续接 | Estimated LOC: 1200
- [ ] **ZS1-184** — 工具循环预算可配置：新增 `--max-tool-iterations N`（1..=10000，默认 1000），对齐成熟 coding agent 的“循环到模型自行停止”行为；8 次的旧常量只保留为历史值，长程任务不再被截断；`Agent::set_max_tool_iterations` 做边界钳制；layer `L3` | Depends: ZS1-180 | Owner scope: 回合工具预算 owner | Owned paths: `src/core.rs`, `tests/config.rs`, `tests/core_session.rs` | Validators: G-CODE；cargo test --locked --test config --test core_session | Rollback: 撤回 flag，恢复 8 次常量 | Estimate: CLI 参数+Agent 预算+钳制+测试 | Estimated LOC: 300
- [x] **ZS1-186** — 模型生命周期状态：registry 增加 ModelStatus（active/alpha/beta/deprecated，无默认值静默填充），纳入 ModelDescriptor digest，`model_status` 投影输出 status，override 严格解码；layer `L3` | Depends: ZS1-184 | Owner scope: 模型目录状态 owner | Owned paths: `src/providers/registry.rs`, `src/core.rs`, `tests/stage1_model_registry.rs` | Validators: G-CODE；cargo test --locked --test stage1_model_registry | Rollback: 撤回 status 字段，保留原 digest | Estimate: 枚举+digest+投影+测试 | Estimated LOC: 400
- [x] **ZS1-187** — Doom-loop 防护：同一回合内重复相同（工具+参数）调用超过阈值时按稳定错误拒绝该次调用并把原因回给模型，回合继续；阈值与计数按回合重置，不跨回合累积；layer `L3` | Depends: ZS1-184 | Owner scope: 工具循环防护 owner | Owned paths: `src/core.rs`, `src/tools.rs`, `tests/core_session.rs` | Validators: G-CODE；cargo test --locked --test core_session | Rollback: 撤回计数，恢复无限制重复 | Estimate: 计数+拒绝+测试 | Estimated LOC: 500
- [ ] **ZS1-188** — 权限规则服务：在审批协调器之上实现规则层（Allow/Deny/Ask + glob patterns + always 规则集 + 同 session 级联 + visible_tools），支持 reject 反馈与 once/always/reject 语义，规则以 session 事件持久化并在恢复时重建；layer `L3` | Depends: ZS1-182 | Owner scope: 权限规则服务 owner | Owned paths: `src/permission.rs`, `src/approval.rs`, `src/core.rs`, `src/lib.rs`, `tests/approval_owner.rs` | Validators: G-CODE；cargo test --locked --test approval_owner | Rollback: 撤回规则层，保留单工具审批 | Estimate: 规则评估+持久化+级联+测试 | Estimated LOC: 1600
- [ ] **ZS1-189** — 子代理权限派生：父规则集派生子会话规则（名称出现即覆盖、deny 优先、不去重、顺序保持），task/todowrite 默认 deny，派生结果在工具 dispatch 前生效且父运行时字段不被原地改写；layer `L3` | Depends: ZS1-188 | Owner scope: 子代理权限 owner | Owned paths: `src/permission.rs`, `src/core.rs`, `src/tool_runtime.rs`, `tests/stage1_tool_batch.rs` | Validators: G-CODE；cargo test --locked --test stage1_tool_batch | Rollback: 撤回派生函数，恢复单一策略 | Estimate: 纯函数+gate 接线+测试 | Estimated LOC: 900
- [ ] **ZS1-190** — Provider 错误分类与稳定码：结构化 HeaderTimeout/StreamError/ParsedApiCallError/ContextOverflow，映射到 BackendError 与 JSONL error_code（context_overflow/provider_api_error/provider_header_timeout/provider_stream_error），仅在未发业务事件且可重试时重试；layer `L3` | Depends: ZS1-184 | Owner scope: provider 错误分类 owner | Owned paths: `src/backend.rs`, `src/protocols/chat.rs`, `src/protocols/responses.rs`, `src/protocols/anthropic.rs`, `src/protocols/google.rs`, `src/protocols/mod.rs`, `src/headless.rs`, `src/protocol.rs`, `tests/headless_protocol.rs` | Validators: G-CODE；cargo test --locked --test headless_protocol | Rollback: 撤回分类，保留字符串错误 | Estimate: 枚举+映射+码+测试 | Estimated LOC: 1200
- [ ] **ZS1-191** — LLM 事件粒度：为 provider 流补齐 step/text/reasoning/tool-input 的 start/delta/end 事件与 usage（cache/reasoning）归一化，core 只消费规范化事件并持久化 assistant turn；layer `L3` | Depends: ZS1-190 | Owner scope: LLM 事件适配 owner | Owned paths: `src/backend.rs`, `src/core.rs`, `src/view_model.rs`, `tests/stage1_chat_stream.rs` | Validators: G-CODE；cargo test --locked --test stage1_chat_stream | Rollback: 撤回细粒度事件，保留文本增量 | Estimate: 事件枚举+适配+测试 | Estimated LOC: 1800
- [ ] **ZS1-192** — 指令服务（AGENTS.md）：发现并合并用户/项目/全局 AGENTS.md 与 openspec/AGENTS.md 指令，去重与 claims 记录，计入同一 context 预算，读取失败不伪造内容；layer `L3` | Depends: ZS1-183 | Owner scope: 指令发现与合并 owner | Owned paths: `src/instruction.rs`, `src/core.rs`, `src/lib.rs`, `tests/stage1_prompt_resources.rs` | Validators: G-CODE；cargo test --locked --test stage1_prompt_resources | Rollback: 撤回指令服务，保留技能注入 | Estimate: 发现+合并+预算+测试 | Estimated LOC: 1100
- [ ] **ZS1-193** — 会话提醒（plan/build）：按 AgentMode 注入有界 system reminder（plan/build/steer 重发），reminder 作为带 metadata 的记录且不伪装成用户输入；layer `L3` | Depends: ZS1-192 | Owner scope: 会话提醒 owner | Owned paths: `src/session_reminders.rs`, `src/core.rs`, `tests/core_session.rs` | Validators: G-CODE；cargo test --locked --test core_session | Rollback: 撤回 reminder 注入 | Estimate: 纯决策+注入+测试 | Estimated LOC: 800
- [ ] **ZS1-194** — 按 session 的运行态控制器：SessionRunState 提供 ensure_running/start_shell/cancel/assert_not_busy 与 busy 事件，按 session 隔离 job 与取消，shutdown 有界等待；layer `L3` | Depends: ZS1-184 | Owner scope: session 运行态 owner | Owned paths: `src/session_run_state.rs`, `src/runtime.rs`, `src/headless.rs`, `tests/headless_protocol.rs` | Validators: G-CODE；cargo test --locked --test headless_protocol | Rollback: 撤回控制器，保留单 job | Estimate: 控制器+隔离+测试 | Estimated LOC: 1300
- [ ] **ZS1-195** — Shell 命令 arity 前缀：纯函数 command_prefix 与 arity 表生成审计键（flags 不计入），仅用于审批预览与事件审计，不改变审批语义；layer `L3` | Depends: ZS1-188 | Owner scope: shell 审批审计 owner | Owned paths: `src/command_arity.rs`, `src/core.rs`, `tests/core_session.rs` | Validators: G-CODE；cargo test --locked --test core_session | Rollback: 撤回前缀，保留完整命令 | Estimate: arity 表+前缀+测试 | Estimated LOC: 500
- [ ] **ZS1-196** — Agent 目录：AgentInfo/AgentMode/AgentCatalog 与 AgentList/AgentGet/AgentGenerate 协议命令（有界、版本化），内置 build/plan/explore 定义与用户覆盖合并；layer `L3` | Depends: ZS1-186 | Owner scope: agent 目录 owner | Owned paths: `src/agent.rs`, `src/protocol.rs`, `src/headless.rs`, `tests/headless_protocol.rs` | Validators: G-CODE；cargo test --locked --test headless_protocol | Rollback: 撤回目录与命令 | Estimate: 目录+协议+测试 | Estimated LOC: 1500
- [ ] **ZS1-197** — Provider 认证方法：通用 API/OAuth 方法注册（prompts/authorize/callback）、按 provider 隔离 pending flow、重复 callback 拒绝、成功写 credential store 且不把 token 写入 session；layer `L3` | Depends: ZS1-190 | Owner scope: provider 认证方法 owner | Owned paths: `src/auth/provider_methods.rs`, `src/auth/mod.rs`, `src/protocol.rs`, `src/headless.rs`, `tests/config.rs` | Validators: G-CODE；cargo test --locked --test config | Rollback: 撤回通用方法，保留 codex 固定流 | Estimate: 方法注册+隔离+测试 | Estimated LOC: 1900
- [x] **ZS1-407** — DAG 编排最小集：`src/dag.rs` 提供 flock 共享存储（`ZENPI_DAG_STORE`）、节点父子/祖孙/兄弟角色寻址（`DagRelation`：parent/grandparent/sibling/child/all）、claim/ack 邮箱（未 ack 自动重投）、子树全绿 close gate（`can_close` 返回未完成后代）、心跳保活（`touch_worker`/`worker_idle_seconds`）与 live spawn 注册表（持有子进程 stdin 可续发 prompt）；工具集 `dag_status`/`dag_send`/`dag_recv`/`dag_finish`/`dag_spawn`，未达标时明确要求保活并派生；配套 opencode/codex/Claude Code agent 通信 1:1 中文学习（55 文件 + 16 目录汇总，8 并发）；layer `L3` | Depends: ZS1-159 | Owner scope: DAG 编排与 worker 保活 owner | Owned paths: `src/dag.rs`, `src/tools.rs`, `src/lib.rs`, `tests/dag.rs`, `tools/learn_agent_comms.sh` | Validators: G-CODE；cargo test --locked --test dag --test tools | Rollback: 撤回 dag 模块与工具，保留原工具集 | Estimate: 存储+角色寻址+claim/ack+close gate+spawn 保活+测试 | Estimated LOC: 1100
- [ ] **ZS1-185** — Resources worker 方块矩阵：观察模式按当前 session 汇总所有 worktree 的 agent worker 总数（2 个 master：PM + Arch，加各 worktree 并发数之和），每个 worker 一个正方形；布局档位固定为 1x2、2x4、4x8、8x16、16x32、32x64、64x128、128x256、256x512…（每档行、列各为上一档 2 倍），worker 总数超过上一档容量时使用下一档；方块按行优先填充并保持正方形，master 与 worktree worker 用不同标识/颜色，选中方块在详情条显示身份与该 worker 的 i/o（若存在）；layer `L3` | Depends: ZS1-175 | Owner scope: Resources worker 方块矩阵 owner | Owned paths: `src/tui.rs`, `tests/tui_interaction.rs` | Validators: G-CODE；cargo test --locked --test tui_interaction | Rollback: 撤回档位矩阵，恢复 2 的幂自适应列数 | Estimate: 档位表+slot 汇总+方块渲染+测试 | Estimated LOC: 900
- [ ] **ZS1-182** — 审批 confirm 分级与拒绝反馈：对齐 opencode 的 permission 确认体验——allow once 直接提交；remember/always 进入二次 Confirm（Enter 确认 / Esc 取消）；reject 进入文本输入（可留空），拒绝原因作为 tool 反馈回给模型；审批视图标题显示当前阶段与按键提示；headless/JSONL 响应同样携带 message；layer `L3` | Depends: ZS1-177,ZS1-180 | Owner scope: 审批确认阶段与拒绝反馈 owner | Owned paths: `src/approval.rs`, `src/tui.rs`, `src/core.rs`, `tests/approval_cap.rs`, `tests/tui_approval_focus.rs` | Validators: G-CODE；cargo test --locked --test approval_cap --test tui_approval_focus --test approval_owner | Rollback: 撤回分级确认，恢复单键 y/n/r | Estimate: 三阶段状态机+拒绝文本+反馈回模型 | Estimated LOC: 900
- [ ] **ZS1-183** — OpenSpec 支持：随 zenpi 提供 openspec skill（`~/.zenpi/skills/openspec/SKILL.md`，仓库副本 `Docs/skills/openspec/SKILL.md`），指导模型按 OpenSpec 三阶段工作流（proposal/tasks/validate、`openspec` CLI、`openspec/AGENTS.md`）工作；`openspec/` 目录存在时优先遵循其 AGENTS.md；shell 工具可直接调用 `openspec` 命令（--auto 下无需审批）；layer `L3` | Depends: ZS1-180 | Owner scope: OpenSpec 工作流接入 owner | Owned paths: `Docs/skills/openspec/SKILL.md`, `src/skills.rs`, `tests/stage1_skill_md.rs` | Validators: G-CODE；cargo test --locked --test stage1_skill_md | Rollback: 撤回 skill 与提示，保留 openspec CLI 手动调用 | Estimate: skill 文件+加载路径+测试 | Estimated LOC: 300
- [ ] **ZS1-180** — `--auto` 全放行模式：新增顶层 `--auto`，把审批策略设为 `Never`（受 SideEffectPolicy 与 Blueprint gate 约束，仍写 approval 审计事件）；该模式必须覆盖 conversation 与 arch 两个独立 owner（根 Agent、`prepare_project_with_options` 新建项目 owner、`ProjectOwnerPool.arch_agent`、TUI 切项目时的 `bind_agent_to_active_project`），使自动放行后工具照常执行、回合正常继续而不停在 pending approval；layer `L3` | Depends: ZS1-176 | Owner scope: CLI 自动放行与双 owner 策略注入 owner | Owned paths: `src/core.rs`, `src/project_workspace.rs`, `src/tui.rs`, `src/approval.rs`, `tests/config.rs`, `tests/project_workspace.rs`, `tests/approval_owner.rs` | Validators: G-CODE；cargo test --locked --test config --test project_workspace --test approval_owner | Rollback: 撤回 --auto 与池级策略注入，恢复 ReadOnly/Always 默认 | Estimate: --auto 解析+策略注入+双 owner 覆盖+自动放行后继续执行 | Estimated LOC: 900
- [ ] **ZS1-181** — 思考中动画：conversation（`busy`）与 arch（`master_busy`）活动期间在 TUI 显示小型动态指示（braille/点阵 spinner + “思考中”），随已有 tick 推进；busy 开始时重置相位；生产渲染路径（render_bentobox 的 prompt 标题、Conversation/Arch pane 标题）必须展示动画，而不是只有旧 render 路径的 ASCII 四帧 spinner；layer `L3` | Depends: ZS1-177,ZS1-179 | Owner scope: 活动指示动画 owner | Owned paths: `src/tui.rs`, `tests/tui_interaction.rs`, `tests/tui_transcript_ux.rs` | Validators: G-CODE；cargo test --locked --test tui_interaction --test tui_transcript_ux | Rollback: 撤回动画，恢复静态 Working 文案 | Estimate: spinner 帧+双区域标题动画+tick 驱动+测试 | Estimated LOC: 700
...
### 3.1.34 二层并发数字与 Resources 观察模式（新增 ZS1-174…176，清单 160 项）

用户要求（2026-09-21，先补蓝图再改代码，以保证 blueprint 与代码一致）：

- **二层并发数字居中与双击编辑（ZS1-174）**：二层每个 worktree 的 `↑ N ↓` 控件，数字 N 在按钮块内水平居中；双击数字直接进入内联数字编辑，只接受有界整数，Enter 提交、Esc 取消，提交结果与键盘调整走同一并发配额校验。
- **Resources 多 headless 网格观察模式（ZS1-175）**：双击 Resources 面板进入全屏观察模式，再次双击返回。观察模式像传统资源管理器的 CPU 方格阵列一样，用正方形格子展示每个 headless worker 的实时状态（phase/CPU/RSS 与 i/o 摘要）。列数必须是 2 的整数次幂；在满足该约束的前提下，列数/行数组合尽量贴近当前显示区域的长宽比；格子保持正方形，剩余空间均匀留白。
- **jerrylookup PR 集成（ZS1-176）**：合并 jerrylookup 的 PR #5–#10（chat stream 空 tool name/id 容错、/new 未决操作、OpenAI wire 输出上限、未知结果解除输入封锁与 transcript 修复、/recovery abandon-all、recovery UX SPEC）；冲突一律以 PR 版本为准；集成后运行对应测试并 push。

验收：逐项实现并补测试（网格模式须有列数为 2 的幂、格子为正方形、双击往返的断言），主控集成后 build 到 `zenpi-dev`。3.1.34 追加不改动此前已接受项的字节与义务。

### 3.1.35 完整热区与 Session 命名（新增 ZS1-177…179，清单 163 项）

用户要求（2026-09-21，先补蓝图再改代码，以保证 blueprint 与代码一致）：

- **完整热区模型（ZS1-177）**：热区必须显式包含 `无热区`，并覆盖 `Conversation`、`Resources`、`Arch`、`Gantt`、`Shell`。每个热区的快捷键与输入行为不同且要成体系：
  - Conversation：正常输入 prompt 与 slash commands；Enter 提交、Ctrl-R 历史、Tab 切区。
  - Resources：上下选择资源块、Enter 进入/展开、双击进入 headless 观察模式；不产生文本输入。
  - Arch：独立输入 prompt 与 slash commands，独立草稿/历史/提交目标。
  - Gantt：支持鼠标滚轮/触控板上下拨动与键盘上下/翻页滚动。
  - Shell：原始 PTY 输入。
  - 无热区：不接收文本；只保留全局键（Tab 切区、Alt 浮层、双击 Resources、双击 Ctrl-C）。
  - footer 显示当前热区及其快捷键提示；Esc 从 Arch 回 Conversation，从 Resources/Gantt 退到无热区。
- **Session 自定义命名与 `-s` 恢复（ZS1-178）**：header 中 logo 右侧、`| workspaces` 左侧显示当前 session 名字，双击可内联重命名；提交时检查与缓存 zenpi session 名字重复，重复则友好提示并重新输入；名字持久化后，`zenpi -s [name|id]` 可直接按名字恢复（无需 hash）。
- **双击 Ctrl-C 强杀（ZS1-179）**：按热区区分 Ctrl-C 语义；强制终止只清理运行态（队列/审批），不破坏 journal，因此 `zenpi -s` 始终可续接。

验收：逐项实现并补测试（热区矩阵、无热区、footer 提示、重名拒绝、`-s` 按名恢复、双击强杀后仍可恢复），主控集成后 build 到 `zenpi-dev`。3.1.35 追加不改动此前已接受项的字节与义务。

### 3.1.36 全放行模式与思考中动画（新增 ZS1-180…181，清单 165 项）

用户要求（2026-09-22，先补蓝图再改代码，以保证 blueprint 与代码一致）：

- **`--auto` 全放行（ZS1-180）**：`zenpi --auto` 把所有需要 approve 的操作直接 allow/yes；conversation 与 arch 两个独立 owner 都要 allow-all；放行后工具必须照常执行、回合继续，不允许停在 pending approval。
- **思考中动画（ZS1-181）**：正在 thinking 时要有小动画显示“正在思考”；生产 TUI（render_bentobox）必须在 conversation 与 arch 活动期间显示动态指示，而不是只在旧渲染路径的 `|/-\`。

验收：逐项实现并补测试（--auto 下双 owner 策略为 Never 且写入工具实际执行、动画帧随时间推进），主控集成后 build 到 `zenpi-dev`。3.1.36 追加不改动此前已接受项的字节与义务。

### 3.1.37 审批 confirm 与 OpenSpec（新增 ZS1-182…183，清单 167 项）

用户要求（2026-09-22）：`--auto` 全放行之外，审批交互要对齐 opencode 的 confirm 体验（once 直接放行、always 二次确认、reject 可带原因反馈给模型）；同时 zenpi 要支持 OpenSpec（opencode/codex 都在用）：随包提供 openspec skill，按 `openspec/AGENTS.md` 三阶段工作流执行，shell 可直接调用 `openspec` CLI。

验收：逐项实现并补测试，主控集成后 build 到 `zenpi-dev`（含 20.214）。3.1.37 追加不改动此前已接受项的字节与义务。

### 3.1.38 工具循环预算（新增 ZS1-184，清单 168 项）

用户要求（2026-09-22）：8 次工具循环上限过小，长程任务会被截断；按成熟 coding 工具的做法，默认给 1000，并提供 `--max-tool-iterations N` 显式调整（上限 10000）。

验收：参数解析、Agent 钳制与长回合行为测试；主控集成后 build 到 `zenpi-dev`（含 20.214）。3.1.38 追加不改动此前已接受项的字节与义务。

### 3.1.39 Resources worker 方块矩阵（新增 ZS1-185，清单 169 项）

用户要求（2026-09-22）：Resources 观察模式对当前 session 汇总所有 worktree 的 agent worker（含 2 个 master：PM 与 Arch），每个 worker 画一个正方形；档位固定 1x2、2x4、4x8、8x16、16x32、32x64、64x128、128x256、256x512…，行列各为上一档 2 倍，超出上一档总量用下一档。

验收：档位选择与方块数测试；本机与 20.214 均 build 并测试通过后 commit push。3.1.39 追加不改动此前已接受项的字节与义务。

### 3.1.40 opencode runtime learn 差距登记（新增 ZS1-186…197，清单 181 项）
...
### 3.1.42 DAG orchestration（新增 ZS1-407，清单 182 项）
...
验收：`src/dag.rs` + 5 builtin + 6 项测试（角色寻址、claim/ack 重投、close gate、心跳保活、spawn 保活 stdin、工具注册）；55 篇文件笔记 + 16 篇目录汇总全绿；本机 build `zenpi-dev`。3.1.42 追加不改动此前已接受项的字节与义务。

注（机器迁移遗留）：本机 pi-mono 检出为 `bbb61e34aaf231639fdaaad1adbd757947034eac`，蓝图声明的 `23282f60782f02b9e22b787e4b22af441454fa16` 在 origin 不可达（`git fetch` 返回 not our ref），G-STAGE 的源 HEAD 校验因此在迁移后仍红；selector 已按本蓝图声明同步（requirement/snapshot/version/baseline_files），源冻结重签与 x 项回执补齐（164 x 中 66 有回执）列为后续 master 维护项，不在本项范围。
