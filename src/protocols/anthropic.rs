...
fn validate_blocks(
    blocks: &[Value],
    deepseek: bool,
) -> Result<(String, Vec<ToolCall>), BackendError> {
    ...
    for b in blocks {
        ...
        match kind {
            ...
            "thinking" => {
                ...
                {
                    return Err(invalid("DeepSeek cannot replay signed Claude thinking"));
                }
                if !deepseek && string(b, "signature")?.trim().is_empty() {
                    return Err(invalid("Anthropic thinking signature missing"));
                }
            }
            "redacted_thinking" => {
                if deepseek {
                    return Err(invalid("DeepSeek does not support redacted thinking"));
                }
...
fn assistant_blocks(
    ...
    if let Some(a) = native.first() {
        ...
        let (text, saved_calls) = validate_blocks(blocks, deepseek)?;
        ...
        if blocks
            ...
            && if deepseek {
                a.get("requested_model").unwrap_or(&a["model"]) != model
            } else {
                a["model"] != model
            }
        {
    ...
    validate_blocks(&blocks, deepseek)?;
    Ok(blocks)
}
...
pub(crate) fn request_body(
    ...
    request_body_inner(request, model, effort, provider, false)
}

pub(super) fn request_body_deepseek(
    request: &CompletionRequest<'_>,
    model: &str,
    effort: Option<&str>,
    provider: &str,
) -> Result<Value, BackendError> {
    request_body_inner(request, model, effort, provider, true)
}

fn request_body_inner(
    request: &CompletionRequest<'_>,
    model: &str,
    effort: Option<&str>,
    provider: &str,
    ...
    for turn in request.turns {
        ...
        let role = match turn.role {
            ...
            TurnRole::Assistant => {
                ...
                blocks = assistant_blocks(turn, provider, model, deepseek)?;
                for b in &blocks {
                    if b["type"] == "tool_use" {
            ...
            TurnRole::Tool => {
                ...
                if deepseek && is_error {
                    return Err(config("DeepSeek does not enforce tool_result.is_error"));
                }
                let mut block =
                    json!({"type":"tool_result","tool_use_id":id,"content":turn.content});
                if !deepseek {
    ...
    if let Some(e) = effort {
        if deepseek {
            let effort = super::deepseek_effort(e)?;
            body["thinking"] =
...
impl Fold {
    ...
    fn event(
        ...
        if kind == "message_start" {
            ...
            if !self.deepseek && model != expected_model {
                return Err(invalid("unexpected Anthropic serving model"));
            }
            self.id = Some(identity(m, "id")?);
            self.model = Some(model);
            self.requested_model = Some(expected_model.to_owned());
            self.usage(&m["usage"])?;
        ...
        match kind {
            ...
            "content_block_delta" | "content_block_stop" => {
                ...
                if kind == "content_block_stop" {
                    ...
                    validate_blocks(std::slice::from_ref(&b.value), self.deepseek)?;
                    b.closed = true;
                    b.partial.clear();
                } else {
                    let d = &v["delta"];
    ...
    fn finish(
        ...
        let mut blocks = self
            ...
            .map(|b| b.value)
            .collect::<Vec<_>>();
        let (mut content, tool_calls) = validate_blocks(&blocks, self.deepseek)?;
        let stop = self
            .stop
            .as_deref()
        ...
        let mut native_history = json!({"type":"native_history","wire":WIRE,"provider":provider,"model":self.model,"blocks":blocks});
        if self.deepseek {
            native_history["requested_model"] = json!(self.requested_model);
        }
...
pub(crate) fn read_response(
    ...
    read_response_inner(body, sse, provider, model, caps, cancelled, sink, false)
}

pub(super) fn read_response_deepseek(
    body: &mut ureq::Body,
    sse: bool,
    provider: &str,
    model: &str,
    ...
    read_response_inner(body, sse, provider, model, caps, cancelled, sink, true)
}
...
fn read_response_inner(
    ...
    if !sse {
        let v = super::read_bounded_json_body(body, cancelled)?;
        ...
        loop {
            ...
            let n = match reader.read(&mut chunk) {
                Ok(0) => break,
                Ok(n) => n,
                Err(e) if super::is_recv_body_poll_timeout(&e) => continue,
